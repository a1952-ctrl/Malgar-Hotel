...
using Noise = MalgarHotel.Audio.NoiseSystem;
using UnityEngine;
...
namespace MalgarHotel.Player
{
    ...
    public class PlayerController : MonoBehaviour
    {
        ...
        [SerializeField] private float baseLookSensitivity = 120f;
        ...
        [SerializeField, Range(0f, 1f)] private float footstepBaseVolume = 0.4f;
        ...
        private void Awake()
        {
            ...
            _originalHeight = Mathf.Max(standingHeight, _characterController.height);
            _originalCenter = _characterController.center;
            _characterController.height = standingHeight;
            _characterController.center = new Vector3(_originalCenter.x, standingHeight * 0.5f, _originalCenter.z);
            ...
            footstepSource.volume = Mathf.Clamp01(footstepBaseVolume);
        ...
        private void Start()
        {
            ...
            {
                _currentMouseSensitivity = GameManager.Instance.MouseSensitivity;
                AssignAudioMixerGroups();
                SetInputBlocked(GameManager.Instance.IsInputBlocked);

                GameManager.Instance.PauseStateChanged += OnPauseStateChanged;
                GameManager.Instance.GameCompleteChanged += OnGameCompleteChanged;
        ...
        private void HandleLook()
        {
            ...
            float lookSpeed = baseLookSensitivity * _currentMouseSensitivity;
        ...
        private void EmitFootstepNoise()
        {
            ...
            float finalIntensity = baseIntensity * Noise.GetSurfaceMultiplier(surface);
            ...
            Noise.EmitNoise(origin, Mathf.Clamp01(finalIntensity), 0, 0.75f, NoiseTag.Footstep);
        }
        ...
        {
            float volume = Mathf.Clamp01(footstepBaseVolume * Random.Range(0.8f, 1.05f));
            ...
            {
                footstepSource.PlayOneShot(_generatedFootstepClip, volume);
            }
        }
        ...
        {
            bool blocked = paused || (GameManager.Instance != null && GameManager.Instance.IsGameComplete);
            SetInputBlocked(blocked);
        }

        private void OnGameCompleteChanged(bool _)
        {
        ...
        private void AssignAudioMixerGroups()
        {
            ...
            if (footstepSource != null && GameManager.Instance.SfxGroup != null)
            {
                footstepSource.outputAudioMixerGroup = GameManager.Instance.SfxGroup;
            }
        }
    }
}
