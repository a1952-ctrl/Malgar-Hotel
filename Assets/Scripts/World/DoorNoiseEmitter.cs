...
namespace MalgarHotel.World
{
    ...
    public class DoorNoiseEmitter : MonoBehaviour
    {
        ...
        {
            NoiseSystem.Instance?.EmitNoise(transform.position, openIntensity, priority, eventLife, NoiseTag.Door);
        }

        public void EmitClose()
        {
            NoiseSystem.Instance?.EmitNoise(transform.position, closeIntensity, priority, eventLife, NoiseTag.Door);
        }
    }
}
